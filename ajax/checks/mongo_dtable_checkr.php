...
if ( !empty($input['sSearch']) ) {
	...
	{
		$cursor = $m_collection->find(array('cid' => new MongoInt32($cid), 'hread' => new MongoRegex( '/'.$input['sSearch'].'/i' )))->sort(array('time' => -1));	
	}
	else
	{
		$start = $_GET['from'] - 30;
		$end = $_GET['to'] + 30;
		$cursor = $m_collection->find(array('cid' => new MongoInt32($cid), 'hread' => new MongoRegex( '/'.$input['sSearch'].'/i' ), 'time' => array('$gt' => new MongoInt32($start), '$lt' => new MongoInt32($end))))->sort(array('time' => -1));		
	}
}
else
{
	...
	{
		$start = time() - 604800;
		$cursor = $m_collection->find(array('cid' => new MongoInt32($cid),'time' => array('$gt' => new MongoInt32($start))))->sort(array('time' => -1));	
	}
	else
	{
		$start = $_GET['from'] - 30;
		$end = $_GET['to'] + 30;
		$cursor = $m_collection->find(array('cid' => new MongoInt32($cid),'time' => array('$gt' => new MongoInt32($start), '$lt' => new MongoInt32($end))))->sort(array('time' => -1));			
	}	
}
