...
if ( !empty($input['sSearch']) ) {
	...
	{
		$cursor = $m_collection->find(array('cid' => new MongoInt32($cid), 'hread' => new MongoRegex( '/'.$input['sSearch'].'/i' )));	
	}
	else
	{
		$start = $_GET['from'] - 30;
		$end = $_GET['to'] + 30;
		$cursor = $m_collection->find(array('cid' => new MongoInt32($cid), 'hread' => new MongoRegex( '/'.$input['sSearch'].'/i' ), 'time' => array('$gt' => new MongoInt32($start), '$lt' => new MongoInt32($end))));		
	}
}
else
{
...
foreach ( $cursor as $doc ) {
	...
	$doc['location'] = '<img src="/images/flags/'.$doc['location'].'.png"> ' . getFullnameForTLD($doc['location']);
